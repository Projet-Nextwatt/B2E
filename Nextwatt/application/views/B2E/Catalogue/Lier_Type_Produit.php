...
<?php
if (isset($rsltupdate)) {
    ?><p align="center"><i class="ace-icon fa fa-exclamation-triangle bigger-125"></i><?php echo($rsltupdate); ?></p> <br/><?php
    }
    echo form_open('CI_catalogue/lier_type_produit_action');
    foreach ($soustypes as $st) {
        echo "<h3 class='header smaller lighter green'>" . $st->nomdevis . "</h3>";
        if (isset($produit[$st->id])) {
            foreach ($produit[$st->id] as $prod) {
                ?>
